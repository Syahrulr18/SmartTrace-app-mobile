		...
		<SafeAreaView className="flex-1" style={{ backgroundColor: "transparent" }}>
			...
			>
				...
				<View className="px-5 pt-4">
					...
					<Text
						className="text-[13px] text-[#64748B] mt-1"
						style={{ fontFamily: "Montserrat-Medium" }}
					>
						Monitor stok, harga, dan status pengiriman Anda
					</Text>

					<View className="flex-row items-center gap-3 mt-5">
						<View className="flex-1 flex-row items-center bg-white rounded-full px-4 py-3 border border-[#E4E7EC] gap-2">
							<SearchIcon />
							<TextInput
				...
				<View className="px-5 mt-6 gap-6">
					{productHistory.map((product) => (
						<View key={product.id}>
							...
							<View className="bg-white rounded-2xl mt-2 p-4 border border-[#E2E8F0] gap-4">
									...
									{product.attributes.map((attribute) => (
										...
										>
											...
											{attribute.label === "Lokasi" ? (
												<TouchableOpacity>
													<Text
														className="text-[13px] text-[#1D4ED8] underline"
														style={{ fontFamily: "Montserrat-SemiBold" }}
													>
														{attribute.value}
													</Text>
